      ...
      <main className="ml-64 flex-1 p-8 grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div key={req.id} className="bg-white p-5 rounded-lg shadow-sm border border-gray-200 space-y-4">
              <div className="flex justify-between items-start">
                ...
                {(req.status === 'Approved' || req.status === 'Closed') && (
                  ...
                  >
                    {({ loading }) => (loading ? 'Loading...' : 'Print PO Slip')}
                  </PDFDownloadLink>
                )}
              </div>

              {/* FINANCIAL RECONCILIATION AREA (Link Dispatch Removed) */}
              {req.status === 'Approved' && (
                <div className="bg-blue-50 p-4 rounded-lg border border-blue-100 grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
                  ...
                  <button 
                    ...
                    className="bg-blue-700 text-white py-2.5 rounded-lg text-xs font-bold hover:bg-blue-800 transition active:scale-95 disabled:bg-gray-400"
                  >
              ...
              {req.status === 'Closed' && (
                <div className="pt-2 border-t flex justify-between items-center text-xs">
                   ...
                   <div className="bg-green-50 text-green-700 px-3 py-1 rounded-full font-bold border border-green-100 uppercase tracking-tighter">
                     Record Finalized
                   </div>
                </div>
              )}
        ...
        <div className="bg-white p-6 rounded-lg shadow-xl border-t-4 border-blue-600 h-fit sticky top-8">
          <h2 className="text-lg font-bold text-gray-800 mb-4 flex items-center">
            <Fuel className="w-5 h-5 mr-2" /> New Requisition
          </h2>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="bg-gray-50 p-3 rounded text-xs text-gray-500 border border-gray-200">
              <div className="flex justify-between mb-1 text-[10px] uppercase font-bold">
                ...
                <span className="font-mono text-gray-400">AUTO-GENERATED</span>
              </div>
            ...
            <div>
              <label className="text-xs font-bold text-gray-500 uppercase">Ambulance Unit</label>
              <select 
                required
                className="w-full mt-1 p-2 border rounded text-sm bg-white outline-none focus:ring-2 focus:ring-blue-500"
            ...
            <div>
              ...
              <input 
                ...
                className="w-full mt-1 p-2 border rounded text-sm bg-gray-100 font-bold text-gray-600" 
              />
            </div>

            <div>
              <label className="text-xs font-bold text-gray-500 uppercase">Requesting Driver</label>
              <select 
                required
                className="w-full mt-1 p-2 border rounded text-sm bg-white outline-none focus:ring-2 focus:ring-blue-500"
            ...
            <div>
              <label className="text-xs font-bold text-gray-500 uppercase">Current Odometer (km)</label>
              <input 
                type="number" 
                required 
                placeholder="000000" 
                className="w-full mt-1 p-2 border rounded text-sm font-mono outline-none focus:ring-2 focus:ring-blue-500" 
            ...
            <button 
              type="submit" 
              className="w-full bg-blue-600 text-white py-3 rounded-lg font-bold shadow-md hover:bg-blue-700 transition-colors active:scale-95"
            >
              SUBMIT REQUISITION
            </button>
          </form>
...
function StatusBadge({ status }: { status: string }) {
  if (status === 'Pending') return <span className="bg-yellow-100 text-yellow-800 px-2 py-1 rounded text-[10px] font-bold flex items-center uppercase"><Clock className="w-3 h-3 mr-1"/> Pending</span>
  if (status === 'Approved') return <span className="bg-blue-100 text-blue-800 px-2 py-1 rounded text-[10px] font-bold flex items-center uppercase"><CheckCircle className="w-3 h-3 mr-1"/> Approved</span>
  if (status === 'Closed') return <span className="bg-green-100 text-green-800 px-2 py-1 rounded text-[10px] font-bold flex items-center uppercase"><FileText className="w-3 h-3 mr-1"/> Finalized</span>
  if (status === 'Rejected') return <span className="bg-red-100 text-red-800 px-2 py-1 rounded text-[10px] font-bold flex items-center uppercase"><AlertCircle className="w-3 h-3 mr-1"/> Rejected</span>
  return null
}